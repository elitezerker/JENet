...
public interface NDR_NOTIFY2_ROUTINE {
    ...
    static MemorySegment allocate(NDR_NOTIFY2_ROUTINE fi, MemorySession session) {
        return RuntimeHelper.upcallStub(NDR_NOTIFY2_ROUTINE.class, fi, constants$853.NDR_NOTIFY2_ROUTINE$FUNC, session);
    }
    static NDR_NOTIFY2_ROUTINE ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (byte _flag) -> {
            try {
                constants$853.NDR_NOTIFY2_ROUTINE$MH.invokeExact((Addressable)symbol, _flag);
            } catch (Throwable ex$) {
