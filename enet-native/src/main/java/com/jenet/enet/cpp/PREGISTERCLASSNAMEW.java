...
public interface PREGISTERCLASSNAMEW {
    ...
    static MemorySegment allocate(PREGISTERCLASSNAMEW fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PREGISTERCLASSNAMEW.class, fi, constants$471.PREGISTERCLASSNAMEW$FUNC, session);
    }
    static PREGISTERCLASSNAMEW ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (java.lang.foreign.MemoryAddress __x0) -> {
            try {
                return (byte)constants$472.PREGISTERCLASSNAMEW$MH.invokeExact((Addressable)symbol, (java.lang.foreign.Addressable)__x0);
            } catch (Throwable ex$) {
