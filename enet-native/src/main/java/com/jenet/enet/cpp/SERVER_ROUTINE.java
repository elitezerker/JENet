...
public interface SERVER_ROUTINE {
    ...
    static MemorySegment allocate(SERVER_ROUTINE fi, MemorySession session) {
        return RuntimeHelper.upcallStub(SERVER_ROUTINE.class, fi, constants$860.SERVER_ROUTINE$FUNC, session);
    }
    static SERVER_ROUTINE ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return () -> {
            try {
                return (int)constants$860.SERVER_ROUTINE$MH.invokeExact((Addressable)symbol);
            } catch (Throwable ex$) {
