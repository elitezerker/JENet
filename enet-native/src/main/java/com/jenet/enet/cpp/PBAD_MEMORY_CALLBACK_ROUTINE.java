...
public interface PBAD_MEMORY_CALLBACK_ROUTINE {
    ...
    static MemorySegment allocate(PBAD_MEMORY_CALLBACK_ROUTINE fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PBAD_MEMORY_CALLBACK_ROUTINE.class, fi, constants$245.PBAD_MEMORY_CALLBACK_ROUTINE$FUNC, session);
    }
    static PBAD_MEMORY_CALLBACK_ROUTINE ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return () -> {
            try {
                constants$245.PBAD_MEMORY_CALLBACK_ROUTINE$MH.invokeExact((Addressable)symbol);
            } catch (Throwable ex$) {
