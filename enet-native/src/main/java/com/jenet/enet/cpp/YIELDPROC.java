...
public interface YIELDPROC {
    ...
    static MemorySegment allocate(YIELDPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(YIELDPROC.class, fi, constants$653.YIELDPROC$FUNC, session);
    }
    static YIELDPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _mciId, int _dwYieldData) -> {
            try {
                return (int)constants$653.YIELDPROC$MH.invokeExact((Addressable)symbol, _mciId, _dwYieldData);
            } catch (Throwable ex$) {
