...
public interface LPFNCANUNLOADNOW {
    ...
    static MemorySegment allocate(LPFNCANUNLOADNOW fi, MemorySession session) {
        return RuntimeHelper.upcallStub(LPFNCANUNLOADNOW.class, fi, constants$1000.LPFNCANUNLOADNOW$FUNC, session);
    }
    static LPFNCANUNLOADNOW ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return () -> {
            try {
                return (int)constants$1000.LPFNCANUNLOADNOW$MH.invokeExact((Addressable)symbol);
            } catch (Throwable ex$) {
